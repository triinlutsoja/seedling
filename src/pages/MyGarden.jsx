...
function PlantCard({ plant, mainPhoto }) {
  const photoUrl = mainPhoto?.dataUrl || plant.photoUrl
  return (
    <Link
      to={`/plant/${plant.id}`}
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="w-14 h-14 rounded-lg bg-green-100 flex items-center justify-center flex-shrink-0 overflow-hidden">
          {photoUrl ? (
            <img src={photoUrl} alt={plant.name} className="w-full h-full object-cover" />
          ) : (
...
export default function MyGarden() {
  ...
  async function loadPlants() {
    ...
    try {
      ...
      // Load main photos for all plants
      const allPhotos = await db.photos.toArray()
      const mainPhotosMap = {}
      allPhotos.forEach(photo => {
        if (photo.isMainPhoto) {
          mainPhotosMap[photo.plantId] = photo
        }
      })
      setMainPhotos(mainPhotosMap)
    } catch (error) {
  ...
  return (
    <div className="min-h-full pb-4">
      ...
      {/* Content */}
      <div className={`px-4 ${showBackupReminder ? 'pt-4' : '-mt-2'}`}>
        {loading ? (
          ...
          <div className="space-y-3">
            {filteredPlants.map(plant => (
              <PlantCard key={plant.id} plant={plant} mainPhoto={mainPhotos[plant.id]} />
            ))}
          </div>
        )}
      </div>
      ...
      {/* Toast notification for navigation messages */}
      {toast && (
        <Toast
          message={toast}
          type="success"
      ...
      {/* Toast notification for backup operations */}
      {toastData && (
        <Toast
